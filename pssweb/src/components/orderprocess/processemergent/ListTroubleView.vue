...
<template>
  <div id="task">
    ...
    <el-row>
      <el-col :span="24">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>抢修装拆列表</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>
    </el-row>
    <!-- 导航结束 -->
    ...
    <!-- 分页结束 -->
      <!-- 修改组件-->
  <update-trouble-view ref="updatepatrol" ></update-trouble-view>
<!-- 详情组件 -->
...
// 引入修改（填写）组件
import updateTroubleView from "@/components/orderprocess/processemergent/UpdateTroubleView.vue";
//引入查看详情组件
import detailView from "@/components/orderprocess/processemergent/DetailView.vue";


export default {
  name: "ListTroubleView",
  data() {
    return {
      formInline: {
        orderno: "",
  ...
  methods: {
    ...
    showupdateProcess(pssOrderProcess) {
      ...
      console.log(pssOrderProcess);
        this.$axios.post('/api/processorder/showadd')
        .then(result=>{
          if(result.data.code===200){
            //改变时间格式
      ...
      this.$refs.showdetail.starttime=this.changetime(new Date(pssOrderProcess.starttime));
       this.$refs.showdetail.endtime=this.changetime(new Date(pssOrderProcess.endtime));
